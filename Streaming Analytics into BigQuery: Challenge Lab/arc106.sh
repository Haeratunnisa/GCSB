...
while true; do
    ...
    if [ "$STATUS" == "Running" ]; then
        ...
        sleep 30
        echo "The Dataflow job is not running, waittt..."
    fi
done


gcloud dataflow jobs run $JOB_NAME-warehouse --gcs-location gs://dataflow-templates-$REGION/latest/PubSub_to_BigQuery --region=$REGION --project=$PROJECT_ID --staging-location gs://$PROJECT_ID/temp --parameters inputTopic=projects/$PROJECT_ID/topics/$TOPIC_NAME,outputTableSpec=$PROJECT_ID:$DATASET_NAME.$TABLE_NAME
